...
/* ***************************************************************************************** */
    ...
    public boolean guardarEmpleadoYSesion(Employees employee, String usuario, String contraseña) {
        ...
        try {
            ...
            String sqlSesion = "INSERT INTO sesiones (usuario, fechaInicio, fechaFin, contraseña)"
                    + " VALUES (?, ?, ?, ?)";
            ...
            if (filasAfectadasSesion > 0) {
                ...
                String sqlEmpleado = "INSERT INTO empleados (idEmpleados, nombreEmpleado, rol, "
                        + "telefono, direccion, email, edad, usuario) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    try {
        ...
        pstmtEmpleado = connection.prepareStatement("UPDATE empleados SET nombreEmpleado = ?, edad = ?, direccion = ?,"
                + " telefono = ?, email = ?, rol = ?, usuario = ? WHERE idEmpleados = ?");
    ...
    public List<Employees> obtenerEmpleadosInactivos() {
    List<Employees> empleadosInactivos = new ArrayList<>();
    try (Connection connection = cn.getConnection(); 
         PreparedStatement pstmt = connection.prepareStatement(
            "SELECT e.idEmpleados, e.nombreEmpleado, e.edad, e.direccion, e.telefono, e.email, e.rol, "
            + "e.usuario, s.contraseña "
            + "FROM empleados e "
            + "LEFT JOIN sesiones s ON e.usuario = s.usuario "
            + "WHERE e.activo = 0")) {  // Filtra solo los empleados inactivos
        ...
        while (rs.next()) {
            ...
            empleado.setSesion(sesion);
            //empleado.setActivo(rs.getBoolean("activo")); // Establecer el estado 'activo'
    ...
    public boolean actualizarEmpleado(Employees empleado) {
    String sql = "UPDATE empleados SET nombreEmpleado = ?, edad = ?, direccion = ?, telefono = ?, email = ?, rol = ?, activo = ? WHERE idEmpleados = ?";
    try (Connection connection = cn.getConnection();
        ...
        pstmt.setString(6, empleado.getRol());
        pstmt.setBoolean(7, empleado.isActivo()); // o setInt dependiendo del tipo de dato
        pstmt.setString(8, empleado.getIdEmpleados());
