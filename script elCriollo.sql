...
select * from mesas;
	...
	('pedido001', 'plato001', 2, 10500),
	('pedido002', 'plato002', 1, 15750),
	('pedido003', 'plato003', 3, 8990);
...
/*************************************  CAMBIOS HYRUM MOLINA  ***************************************/

-- ----------------------------------------------------------------------------------------------------

-- ALTERACION DE TABLAS A MANOS DE HYRUM MOLINA PARA EL TRIGGER 

	ALTER TABLE empleados ADD COLUMN activo TINYINT(1) DEFAULT 1;
	ALTER TABLE sesiones ADD COLUMN activo TINYINT(1) DEFAULT 1;
	ALTER TABLE pedidos ADD COLUMN empleadoInactivo TINYINT(1) DEFAULT 0;
	ALTER TABLE reportes ADD COLUMN empleadoInactivo TINYINT(1) DEFAULT 0;
	ALTER TABLE historialVentas ADD COLUMN empleadoInactivo TINYINT(1) DEFAULT 0;
...
CREATE TRIGGER inactivar_empleado 
...
BEGIN
    ...
    UPDATE empleados
    SET activo = 0
    WHERE idEmpleados = OLD.idEmpleados;

    -- Inactivar la sesión del empleado
    UPDATE sesiones
    SET activo = 0
    WHERE usuario = OLD.usuario;

    -- Actualizar el estado en las tablas relacionadas
    UPDATE pedidos
    SET empleadoInactivo = 1
    WHERE empleadosId = OLD.idEmpleados;
    ...
    -- Repite la misma lógica para otras tablas si es necesario
END//
